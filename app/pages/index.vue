<template>
    <main class="main" v-if="pageData && pageData.length > 0">
      ...
      <ourClientsSection :sectionData="pageData[0]?.acf?.sekcziya_4_nashi_klienty" />
      ...
      <section class="clients-about-us" v-if="pageData[0].acf.sekcziya_5_otzyvy.zagolovok">
        <div class="container">
          ...
          <reviewSlider :sliderData="pageData[0].acf.sekcziya_5_otzyvy.otzyvy" />
...
const { data: _carsCategoryes } = await useFetch(
  `${store.serverUrlDomainRequest}${urlCat}`
);

// Сортируем категории в порядке categoriesIds (WordPress API не сохраняет порядок include)
const idsOrder = categoriesIds.map(c => typeof c === 'object' && c != null ? (c.id ?? c.term_id ?? c.ID) : c);
const carsCategoryes = computed(() => {
  const cats = _carsCategoryes.value || [];
  if (!idsOrder.length) return cats;
  return [...cats].sort((a, b) => idsOrder.indexOf(a.id) - idsOrder.indexOf(b.id));
});
